...
where {
        ...
        let aad_buffer_len = header_len + meta_len;
        ...
        // remove tag and header
        let payload_len = self.data.len() - key.cipher_suite.auth_tag_len - header_len;
        let payload = &io_buffer[..payload_len];
        let meta_data = &aad_buffer[..meta_len];
