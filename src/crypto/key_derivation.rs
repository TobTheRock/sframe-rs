...
///
/// Implement this (alongside [`KeyDerivation`]) to use a custom crypto backend with the
/// [`crate::ratchet`] key store.
pub trait Ratcheting {
    /// Ratchets the given base key forward, producing new key material.
...
mod test {
    ...
    fn ratchets_key(cipher_suite: CipherSuite) {
        ...
        let new_material = Kdf::ratchet(cipher_suite, &original_material).unwrap();
